...
class CustomListView(generics.ListCreateAPIView):
    ...
    def get(self, request, *args, **kwargs):
      twitter_id = request.GET.get('user_id')
      twitter_token = request.GET.get('oauth_token')
      twitter_token_secret = request.GET.get('oauth_token_secret')
      twitter_screen_name = request.GET.get('screen_name')
      ...
      if(Register.objects.filter(twitter_id=twitter_id).exists()):
        obj=Register.objects.get(twitter_id=twitter_id)
        details.append(
                        {
                          'status':200,
        ...
        if(User.objects.filter(username=twitter_id).exists()):
         User.objects.filter(username=twitter_id).delete()



        user=User.objects.create(username=twitter_id,password="foodromeo")
       ...
       # from .tools import get_access_token
        ...
        Register.objects.create(access_token=token,twitter_id=twitter_id,twitter_token=twitter_token,twitter_token_secret=twitter_token_secret,twitter_screen_name=twitter_screen_name)
